...
void loop() {
    ...
    //send one letter command
    if (Serial.available()){
        rlen = Serial.readBytes(buf,BUF_SIZE);
        Serial.print("Received form Serial: ");
        Serial.println(buf);
        index_in_alphabet = alphabet.indexOf(buf[0]);
        if (rlen == 1 && index_in_alphabet>=0){
            String message = "";
            message.concat(buf[0]);
            sendUARTmsg(message);
        }
    }
    // to know when tea is ready

    read_pin = digitalRead(TE_READY);
    if (read_pin == HIGH){
        // doing security wait because else it spams commands
        if(millis()-last_y>=5000UL){
            last_y=millis();
            String message = "";
            message.concat('y');
            sendUARTmsg(message);
